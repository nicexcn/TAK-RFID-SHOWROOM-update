    ...
    <div className="relative overflow-hidden" style={{
      ...
      {mode === "idle" && surveyQr && (
        <div className="absolute" style={{ bottom: 28, left: "50%", transform: "translateX(-50%)", background: "rgba(255,255,255,0.95)", borderRadius: 16, padding: "12px 18px", display: "flex", alignItems: "center", gap: 14, boxShadow: "0 6px 24px rgba(0,0,0,0.28)" }}>
          ...
          <div>
            <p style={{ color: "var(--color-text)", fontSize: 17, fontWeight: 600 }}>สแกนเพื่อให้คะแนน</p>
            <p style={{ color: "var(--color-text-muted)", fontSize: 13 }}>Rate your visit</p>
          </div>
        </div>
      )}
          ...
          <div className="absolute bottom-12 right-12 flex items-center gap-3">
            {products.length > 1 && (
              <div className="flex items-center gap-2">
                {products.map((_, i) => (
                  <div key={i} className="rounded-full transition-all" style={{ width: i === idx % products.length ? 22 : 8, height: 8, background: i === idx % products.length ? "var(--color-surface)" : "rgba(255,255,255,0.4)" }} />
                ))}
              </div>
            )}
                ...
                {products.map((p, i) => {
                  ...
                  return (
                    <div key={(p.rfidTag || p.name) + i} className="flex items-center gap-2 px-1.5 py-1 rounded text-[11px]"
                      style={{ background: isCur ? "rgba(16,185,129,0.18)" : "transparent", color: isCur ? "var(--color-surface)" : "rgba(255,255,255,0.6)" }}>
                      <span style={{ width: 6, height: 6, borderRadius: 9, flexShrink: 0, background: isCur ? "var(--color-success)" : "rgba(255,255,255,0.3)" }} />
                      <span className="truncate">{i + 1}. {p.name}</span>
                      {isCur && <span className="ml-auto text-[9px] whitespace-nowrap" style={{ color: "var(--color-success)" }}>▶ Now showing</span>}
                    </div>
                  );
                })}
              </div>
