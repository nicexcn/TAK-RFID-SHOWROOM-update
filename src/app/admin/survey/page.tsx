...
export default function SurveyResultsPage() {
  ...
  if (state === "loading") return <p style={{ color: "#9f886c" }}>Loading…</p>;
    ...
    <div>
      <div className="flex items-start justify-between mb-6 gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "#9f886c" }}>{data.total} response{data.total === 1 ? "" : "s"}</p>
        </div>
      ...
      {data.total === 0 ? (
        <div className="p-8 rounded-xl text-center" style={{ background: "#fff", border: "1px solid #e6e5d8" }}>
          <p className="text-sm mb-2" style={{ color: "#9f886c" }}>No responses yet.</p>
          <p className="text-xs" style={{ color: "#cdc3ad" }}>Share the survey link: <span style={{ fontFamily: "monospace" }}>{surveyUrl}</span></p>
        </div>
      ) : (
  ...
  return (
    <div>
      <div className="flex items-baseline gap-2 mb-3">
        ...
        <span className="text-xs" style={{ color: "#9f886c" }}>avg / {r.scale} · {r.count} rated</span>
      </div>
      <div className="space-y-1.5">
        {Array.from({ length: r.scale }, (_, i) => r.scale - i).map((n) => (
          <div key={n} className="flex items-center gap-2">
            <span className="text-xs w-3 text-right" style={{ color: "#9f886c" }}>{n}</span>
            ...
            <span className="text-xs w-6 text-right" style={{ color: "#9f886c" }}>{r.dist[n] || 0}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="space-y-1.5">
      {rows.map((x) => (
        <div key={x.name} className="flex items-center gap-2">
          ...
          <span className="text-xs w-8 text-right" style={{ color: "#9f886c" }}>{x.count}</span>
        </div>
      ))}
      <p className="text-[11px] pt-1" style={{ color: "#cdc3ad" }}>{r.type === "checkbox" ? "multiple choice" : "single choice"} · {total} responses</p>
    </div>
  );
}

function TextView({ r }: { r: TextResult }) {
  if (r.comments.length === 0) return <p className="text-sm" style={{ color: "#cdc3ad" }}>No comments</p>;
  return (
