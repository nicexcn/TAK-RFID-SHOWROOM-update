...
export default function ProductGallery({
  ...
  return (
    <div>
      ...
      {error && <p className="text-sm mt-2" style={{ color: "var(--color-danger-soft)" }}>{error}</p>}

      {/* Image grid */}
      {images.length === 0 ? (
        <div className="mt-3 rounded-xl text-center py-10" style={{ border: "1px dashed var(--color-border)" }}>
          <p className="text-sm" style={{ color: "var(--color-text-subtle)" }}>No images yet</p>
        </div>
      ) : (
          ...
          {images.map((img, index) => (
            <div
              ...
              style={{
                ...
                outline: overIndex === index && dragIndex !== index ? "2px dashed var(--color-primary)" : "none",
                outlineOffset: -2,
            ...
            >
              ...
              {index === 0 ? (
                ...
                <div
                  ...
                  style={{ background: "rgba(0,0,0,0.5)", color: "var(--color-surface)" }}
                >
                  {index + 1}
                </div>
              )}
